...
async function abrirEditarCliente(pk) {
    try {
        ...
        mostrarToastExito("No se pudieron cargar los datos del client", "error");
    }
}
...
async function abrirModalDetalleCompleto(pk) {
    try {
        ...
        if (mobiliariosSpan && data.mobiliarios && data.mobiliarios.length > 0) {
            ...
            mobiliariosSpan.textContent = 'No hay mobiliario';
        }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (btnGuardarCliente) {
        btnGuardarCliente.addEventListener("click", async function() {
            ...
            try {
                const response = await fetch("/recepcion/historial/", {
                    method: "POST",
                    body: formData
                });
    ...
    document.querySelectorAll(".editar-reservacion").forEach((btn) => {
        btn.addEventListener("click", async function () {
            ...
            try {
                ...
                document.getElementById("edit-evento-nombre").value = data.nombre_evento || "";
                document.getElementById("edit-evento-descripcion").value = data.descripcion || "";
    ...
    const btnCancelarReservacion = document.getElementById("btn-cancelar-reservacion");
    if (btnCancelarReservacion) {
        btnCancelarReservacion.addEventListener("click", async function () {
            const pk = this.dataset.pk;
            if (!pk) return;
